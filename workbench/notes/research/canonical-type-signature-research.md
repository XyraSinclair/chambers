...
- `workbench/notes/ideation/02-private-cognitive-labor-market.md` defines the economic object: accepted, typed, provenance-bearing cognitive deltas, not raw access or persuasive prose.
- `workbench/notes/ideation/03-egress-and-immune-system.md` defines the hard security stance: everything observable outside owner control is egress; logs, errors, timings, paths, examples, embeddings, and refusal reasons are not harmless.
- `workbench/notes/ideation/06-canonical-type-signatures.md` is the current type-signature lens and should remain the canonical ideation document.
