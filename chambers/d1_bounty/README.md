...
```
python3 -m chambers.d1_bounty.run                 # the five lanes
python3 -m pytest chambers/d1_bounty/ -q          # 17 tests
python3 -m unittest chambers.d1_bounty.test_d1 -v
```

Per-lane court files (hash-chained lane ledger, charge-kernel ledger, findings,
egress report, receipt) land under
`chambers/d1_bounty/out/court/<lane>/`; `validate_courtfile` re-verifies
