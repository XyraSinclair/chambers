...
laws, the cut bound. Above: the stories (`workbench/notes/stories/`, `workbench/notes/STORIES.md`) — nine
...
(`workbench/notes/stories/frontier-lab-ip-mediation.md`, `workbench/notes/frontier/ip-trades/README.md`).
...
3. **Epistemic lane** — `prov` / `trust` / `est` / `unprov`: what backs the
   ...
   `workbench/notes/frontier/ip-trades/valuation-and-research.md`. *Drift, reported not
...
4. **Contingency closure** — `in-ch` (the contingent outcome IS an
   ...
   can express one), `—` (no contingent leg). `workbench/notes/stories/gardener.md`;
   `ASSURANCE.md` L2.
...
6. **Extraction posture** — `1shot` (a single crossing; repetition has no
   ...
   `workbench/notes/stories/frontier-lab-ip-mediation.md`, the meter under adversarial
   pressure.
...
| trust root class (`workbench/notes/frontier/ip-trades/README.md` §3.1) | epistemic lane (what `trust` names) |
...
| value-density regime (`workbench/notes/frontier/private-data-moats/lens-economics.md` §1.3) | extraction posture (whether the meter's bound is honest) |
| attack verdict PREVENTED/PRICED/RECORDED/UNPRICEABLE | closure + extraction (what the substrate does under abuse) |
| moat invariants M1–M5 (`workbench/notes/frontier/private-data-moats/lens-types.md`) | extraction posture (the lifetime-accounting laws behind `meter`) |
| account key families (kernel PROTOCOL) | extraction posture (the ledger keys implementing it) |
| declassification 4-cube (`workbench/notes/research/mediation-literature.md` Vein 2) | alphabet (what), silo (who), alphabet's schedule proviso (when), rung (where) |
...
| oracle mechanicalness (in-chamber vs attested vs refused) | this IS axis 4's source (`workbench/notes/stories/gardener.md`) |
| ObservableClass × precision × cadence atlas (`workbench/notes/autoresearch/2026-06-29-orthogonal-type-systems/01-orthogonal-type-atlas.md` §6) | alphabet class — the concrete enumeration checklist the calculus proviso needs; currently ignored by CALCULUS/STRUCTURE (§6) |
| gain-function class (`workbench/notes/FRAMEWORKS.md` F1) | alphabet class (design-time input: the alphabet is designed against a declared gain function) |
| schedule discipline (fixed/bucketed/padded) | alphabet class (timing is a channel the ceiling never sees) |
| deal-intimacy ladder (`workbench/notes/stories/frontier-lab-ip-mediation.md`) | not an axis: a path through the spine, alphabet widening and silo deepening per step |
...
(`workbench/notes/frontier/ip-trades/README.md` §5), the **two-silo chamber run mode**
...
| `judge` (evidence → closed verdict) | closed | self/2T | trust | — | R1 | meter | IMPL | A | `primitives/CALCULUS.md` §3(ii); `workbench/ip_trade_sim/test_calculus_bound.py` |
...
| `gatePub` (public-inputs envelope) | closed | — | prov (L5) | — | any | 1shot | IMPL | A | `primitives/CALCULUS.md` §3(iv); `workbench/notes/stories/MARKETS.md` §6; `chambers/chamber.py` |
...
| In-chamber derivation / annotation | — | self | prov (zero-charge, `chambers/lean/ChargeKernel/Algebra.lean`) | — | any | — | SPEC | B(build) | `LICENSING.md` rights 1–2; `workbench/notes/stories/gardener.md` |
| Elicitation (typed questions to an owner) | schema | 2T | trust | — | R1 | meter | NAMED | B(schema) | `workbench/notes/stories/party-matchmaker.md` Act 1; G12 |
| In-chamber reframing (never crosses) | — | self | trust | — | any | — | NAMED | B(build) | `workbench/notes/stories/attention-guardian.md` |

### B. Verdict and judgement operations

| Operation | alph | silo | lane | closure | rung | extr | status | tier | source |
|---|---|---|---|---|---|---|---|---|---|
| Overlap detection (trit / IPVerdict ≈ 9.72 bits) | closed | 2T | trust | — | R2 | meter | SPEC; verdict channel IMPL in sim | B(2-silo) | `primitives/CALCULUS.md` §7; `workbench/notes/stories/frontier-lab-ip-mediation.md` Deal 1; `workbench/notes/stories/MARKETS.md` §5 |
| Result verification (score ≥X in sealed verifier) | closed | 3C | prov (TEE) / trust (sim) | in-ch | R3 | meter | IMPL sim / SPEC real | B(TEE) | `workbench/notes/frontier/ip-trades/README.md` §2 §5; `workbench/ip_trade_sim/SIM.md` |
| Method verification (causality/novelty/transfer) | — | — | unprov | refused | — | — | NAMED-as-refusal | **C** | `workbench/notes/frontier/ip-trades/README.md` §2 §6.1; `primitives/CANON.md` IPTRADE_LAWS |
| Verified partition (proven[]/trusted[]/unprovable[]) | closed | any | (the lane machinery itself) | — | any | — | SPEC | B(build) | `primitives/iptrade.ts` via `workbench/notes/frontier/ip-trades/README.md` §2 |
| Novelty / OOD estimation (price haircut, never a gate) | declared | self | est | — | R1 | meter | IMPL | B(est) | `workbench/notes/frontier/ip-trades/valuation-and-research.md`; `workbench/ip_trade_sim/novelty.py` |
| Ranking (full order charges log₂ n!) | closed | self | trust | — | R2 | meter | IMPL | A | `workbench/notes/frontier/private-data-moats/lens-economics.md` §1.2; `workbench/cardinal_wedge/run_sort_metered.py` |
...
| Redact-summarize (word-capped diligence prose) | declared | self | trust | — | R1 | meter | IMPL | B(declared) | `chambers/CHAMBER.md` (output law); `workbench/notes/stories/MARKETS.md` §1 |
...
| Reference check (~60-bit schema + subject consent) | schema | 2T+subject | trust | — | R1 | meter | SPEC | B(schema) | `workbench/notes/stories/reference-check.md`; `workbench/notes/stories/MARKETS.md` §2; G4 |
| Match card projection (13-bit card, no identity) | closed | 2T | trust | — | R2 | meter | IMPL | A | `workbench/notes/stories/party-matchmaker.md` Act 3; `chambers/kernel/demo_attention_notify.py`; runtime `match_card` golden bundle |
| Fit / complement / risk / non-relation judgements | closed | kT | trust | — | R1 | meter | SPEC; intro path IMPL | B(build) | `primitives/CANON.md` MEDIATION_LAWS; `workbench/notes/stories/consult-storyweaver.md`; `workbench/intro_clearing/` |
| Defect hint (~20-bit card) vs raw patch (8 bits/byte) | schema vs declared | 2T | trust | — | R1 | meter | NAMED | B(schema) | `workbench/notes/stories/gardener.md` |
| Negative-knowledge advisory (searched-and-empty) | schema | self | est | ext (bonded coverage) | R1 | meter | NAMED | B(est) | `workbench/notes/frontier/private-data-moats/lens-economics.md` C6, M3; `workbench/notes/stories/frontier-lab-ip-mediation.md` |
| Technique sketch (class/applicability/effect-size enums) | schema | 2T | trust | — | R1 | meter | NAMED | B(schema) | `workbench/notes/stories/frontier-lab-ip-mediation.md` Deal 2 stage 1 |
| Mediated reproduction verdict (third attested enclave) | closed | 3C | trust | in-ch | R3 | meter | NAMED | B(R3) | `workbench/notes/stories/frontier-lab-ip-mediation.md` Deal 2 stage 2 |
| Eval-conformance buckets to regulator | closed | 3R | trust | — | R1 | meter | NAMED; accounting shape exercised (`workbench/d1_bounty/`) | B(build) | `workbench/notes/stories/frontier-lab-ip-mediation.md` Deal 3 |
| Synthesis over leaf judgements (closure-charged) | closed | kT | trust | — | R1 | meter | NAMED | B(build) | `workbench/notes/stories/coagent-economy.md`; P-codes |
| Committed alpha attestation (hash now, score later) | closed | self | est | ext (neutral harness vs public prices) | R2 | meter | NAMED | B(est) | `workbench/notes/stories/MARKETS.md` §3 |
| Cohort aggregate (N-party, real DP ε) | declared | kT | est | — | R1 | meter | NAMED | B(est) | `workbench/notes/stories/MARKETS.md` §7 |
| Expert confidential consult (typed severity verdict) | schema | 2T | trust | ext (audited samples) | R1 | meter | NAMED | B(schema) | `workbench/notes/stories/MARKETS.md` §4 |
| Oracle-approved patch acceptance (quantized bands) | closed | 2T | trust | in-ch | R2 | meter | SPEC | B(build) | `workbench/notes/autoresearch/2026-07-01-premier-use-cases/README.md` §3; `primitives/market.ts`, `primitives/pricing.ts` |
| CI-verdict-as-oracle (pay-on-green) | closed | 2T | trust | in-ch | R2 | meter | SPEC; kernel binding IMPL | B(build) | `workbench/notes/stories/gardener.md`; kernel charge_ids law |

### C. Verification, attestation, audit

| Operation | alph | silo | lane | closure | rung | extr | status | tier | source |
|---|---|---|---|---|---|---|---|---|---|
| Outcome attestation (bonded, quorum'd, slashable) | closed | ext parties | trust | ext (it IS the ext-closure machinery) | R1 | meter | IMPL | B(ext) | charge-settlement/2, `ASSURANCE.md` L2; `workbench/notes/stories/party-matchmaker.md` Act 4 |
| Retroactive appreciation tap (glad/neutral/noise) | closed | 2T | trust | ext | R1 | meter | NAMED | B(ext) | `workbench/notes/stories/attention-guardian.md` |
...
| Estimator adversarial probe (sound + tight over 1472 emissions) | — | self | trust (L3 empirical) | — | R1 | — | IMPL | A | `ASSURANCE.md` L3; `workbench/d1_bounty/estimator_probe.py` |
...
| Peer-prediction quality scoring | closed | kT | trust | — | R1 | meter | IMPL sim | B(build) | `workbench/notes/FRAMEWORKS.md` F5; `workbench/peer_sim/` |
...
| Reuse attestation (licensee-signed build provenance) | declared | self | trust | ext | R1 | 1shot | SPEC | B(ext) | `workbench/notes/frontier/ip-trades/README.md` §3.2 |
| Reuse evidence (contestable exhibit, never a boolean) | schema | 2T | est | ext | R1 | 1shot | SPEC | B(est) | `workbench/notes/frontier/ip-trades/README.md` §3.2 |
| Fuzzy/distance-aware PSI (crypto overlap count) | closed | 2T | prov | — | R1 | 1shot | NAMED | B(build) | `workbench/notes/research/mediation-literature.md` Vein 4 |

### D. Matching, introduction, attention

| Operation | alph | silo | lane | closure | rung | extr | status | tier | source |
|---|---|---|---|---|---|---|---|---|---|
| Priced introduction clearing | closed | 2T | trust | in-ch | R1 | meter | IMPL (contingent-fee leg = named residue, `PARTY_LANE_GAPS`) | A | `workbench/notes/autoresearch/2026-07-01-premier-use-cases/README.md` §2; PRICING_LAWS; `workbench/intro_clearing/` |
| Attention notification (credits AND bits, atomic) | closed | 2T+3rd | trust | in-ch | R1 | meter | IMPL | A | `chambers/kernel/attention_node.py`, `chambers/kernel/demo_attention_notify.py`; `workbench/notes/stories/party-matchmaker.md` Act 3 |
| Denominator control (bucketed pool sizes) | closed | kT | trust | — | R1 | meter | SPEC | B(build) | MATCHING_LAWS; `workbench/notes/autoresearch/2026-06-29-orthogonal-type-systems/01-orthogonal-type-atlas.md` §11 |
| Bilateral reveal / identity widening | closed | 2T | prov (`chambers/lean/ChargeKernel/Widening.lean`) | — | R1 | 1shot (one-way) | SPEC; algebra IMPL | B(build) | `workbench/notes/stories/consult-storyweaver.md` runs 2/6; COALITION_LAWS |

### E. Trade and negotiation lifecycle

| Operation | alph | silo | lane | closure | rung | extr | status | tier | source |
|---|---|---|---|---|---|---|---|---|---|
| Claim commitment (hash first, salted) | closed | self | prov | — | R1 | 1shot | IMPL sim / SPEC | B(build) | `workbench/notes/autoresearch/2026-07-01-premier-use-cases/README.md` §1; `workbench/ip_trade_sim/SIM.md`; `primitives/negotiation.ts` |
| Staged reveal ladder (reciprocity-gated) | schema | 2T | trust | — | R1 | campgn (walk-away timing is an emission) | SPEC | B(schema) | NEGOTIATION_LAWS; `workbench/notes/autoresearch/2026-07-01-premier-use-cases/README.md` §1 |
| Black-box probe (the distillation channel) | declared | 2T | est | — | R1 | campgn | IMPL sim | B(declared) | `workbench/ip_trade_sim/SIM.md` |
| Appraisal / valuation (benchmark-normalized marginal) | declared | self | est | — | R1 | meter | IMPL sim | B(est) | `workbench/ip_trade_sim/economics.py` |
| Price negotiation (metered rounds, commitments persist) | declared | 2T | est | — | R1 | meter | IMPL sim | B(declared) | `workbench/ip_trade_sim/price_debate.py`, `workbench/ip_trade_sim/SIM.md` |
| Atomic settlement / key-for-payment swap | closed | 2T + consortium | trust (regime-typed) | in-ch | R1–R3 by regime | 1shot | SPEC | B(build) | `workbench/notes/frontier/ip-trades/README.md` §3.3 §5 |
| Paid method reveal (fills a fresh consented ceiling) | declared | 2T | trust | — | R1 | 1shot | IMPL sim | B(declared) | `workbench/notes/stories/frontier-lab-ip-mediation.md` Deal 2.4; `workbench/ip_trade_sim/SIM.md` |
| Royalty licensing (consent-first LicenseGrant spine) | schema | 2T | trust | ext | R1 | meter | SPEC | B(ext) | `workbench/notes/frontier/ip-trades/README.md` §3.2 |
| Refusal with evidence (typed refusal record + priorityCommit) | closed | self | trust | — | R1 | 1shot | SPEC | B(build) | `workbench/notes/frontier/ip-trades/valuation-and-research.md` Truth 1 |
| Barter fairness oracle (dual metered OOD estimates) | declared | 2T | est | — | R1 | meter | NAMED | B(est) | `workbench/notes/frontier/ip-trades/valuation-and-research.md` first wedge |
...
| Lifetime (source × reader) exposure accounting + incident latch | prov | — | — | IMPL | A | COALITION_LAWS; `chambers/kernel/`; `workbench/notes/stories/frontier-lab-ip-mediation.md` |
...
| Derivation declaration + provenance-closure charging | prov | — | — | IMPL | A | `workbench/notes/frontier/private-data-moats/lens-types.md` §3; G14; `chambers/lean/ChargeKernel/ProvenanceCompleteness.lean` |
| Attribution split (exact integer Shapley) | prov (`chambers/lean/ChargeKernel/Attribution.lean`) | — | — | IMPL | A | `workbench/notes/stories/alpha-sharing.md`; `chambers/kernel/attribution.py` |
...
| Entropy-pool payment batching (anonymity set stated, k honest) | trust | — | meter | SPEC | B(G15) | `primitives/mediation.ts` laws; `workbench/notes/stories/frontier-lab-ip-mediation.md` Deal 2 |
...
| Erasure tombstone (destroys bytes, never others' proofs) | trust | — | 1shot | SPEC | B(build) | CORE_LAWS; `workbench/notes/frontier/private-data-moats/lens-types.md` M1 |
| Export / portability (exposure history survives the divorce) | trust | — | 1shot | NAMED (G7, unexercised) | B(build) | `workbench/notes/frontier/private-data-moats/lens-types.md` M5 |
...
| Fiduciary legibility query (whose agent is it) | prov (fold arithmetic) | — | — | NAMED | B(build) | `workbench/notes/stories/attention-guardian.md` |
| Moat residual statement (per-pair residual sentence) | trust | — | — | SPEC | B(schema) | `workbench/notes/frontier/private-data-moats/lens-types.md` §3(3) |
| Entropy re-declaration (owner-signed, monotone-down) | — | — | — | NAMED (missing event kind) | **C** until the G13 event kind exists | `workbench/notes/stories/frontier-lab-ip-mediation.md` |
...
| Canonicality / admission review (requested vs justified capacity) | — | any | trust | — | R1 | — | SPEC | B(build) | `primitives/CANON.md` MEDIATION_LAWS; `workbench/notes/stories/consult-storyweaver.md` runs 1/2/7 |
...
| Subject consent gate + response right | schema | 2T+subject | trust | — | R1 | meter | NAMED (G4) | B(build) | `workbench/notes/stories/reference-check.md` |
...
`workbench/notes/stories/frontier-lab-ip-mediation.md`, `workbench/notes/frontier/ip-trades/`,
`workbench/ip_trade_sim/SIM.md`), each step read off the table above.
...
- *The TEE first-wedge* (`workbench/notes/frontier/ip-trades/README.md` §5) gates step 9,
...
- Alphabet- and lane-priced B steps, buildable without new infrastructure:
  ...
  exercised by `workbench/d1_bounty/`).
...
Why this case is premier (`workbench/notes/stories/frontier-lab-ip-mediation.md`, closing
...
- **Freshness/staleness attestation** — attesting a claim's vintage against
  ...
  (`workbench/notes/stories/frontier-lab-ip-mediation.md` G13;
  `workbench/notes/frontier/ip-trades/valuation-and-research.md`).
- **The arXiv-reading estimator institution** — declaring
  ...
  `workbench/ip_trade_sim/novelty.py`.
...
- **Two lane systems, never unified**: derived/declared (charge channels,
  ...
  `workbench/notes/frontier/ip-trades/valuation-and-research.md` added the fourth,
...
- **`entropy_bits` vs `subject_entropy_mbits`** — unit and name drift
  across the sim/kernel boundary (`workbench/ip_trade_sim/types.py` vs
  kernel/stories).
- **The atlas ObservableClass × precision × cadence taxonomy**
  (`workbench/notes/autoresearch/2026-06-29-orthogonal-type-systems/01-orthogonal-type-atlas.md`
